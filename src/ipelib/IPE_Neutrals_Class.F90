MODULE IPE_Neutrals_Class
  ...
  TYPE IPE_Neutrals
    ...
    CONTAINS
      ...
      ! PRIVATE Routines
      PROCEDURE, PRIVATE :: INTERFACE__FIXED_GEO_to_MID_LAT_IONOSPHERE
...
CONTAINS
  ...
  SUBROUTINE INTERFACE__FIXED_GEO_to_MID_LAT_IONOSPHERE ( neutrals, grid, fixed_hts_in_km, geo_grid_longitudes_degrees, geo_grid_colatitudes_degrees, &
                                                          O_density_geo,O2_density_geo,N2_density_geo, H_density_geo,HE_density_geo,N_density_geo, &
                                                          Neutral_temp_geo,Neutral_temp_inf_geo, VnX_meridional_geo,VnY_zonal_geo )
  IMPLICIT NONE

  CLASS( IPE_Neutrals ), INTENT(inout) :: neutrals
  TYPE( IPE_Grid ), INTENT(in)         :: grid
  REAL(prec), INTENT(in)               :: geo_grid_longitudes_degrees(N_Longitudes)
  REAL(prec), INTENT(in)               :: geo_grid_colatitudes_degrees(N_Latitudes)
  REAL(prec), INTENT(in)               :: fixed_hts_in_km(N_heights)
  REAL(prec), INTENT(in)               :: O_density_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: O2_density_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: N2_density_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: H_density_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: HE_density_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: N_density_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: Neutral_temp_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: Neutral_temp_inf_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: VnX_meridional_geo(N_heights,N_Latitudes,N_longitudes)
  REAL(prec), INTENT(in)               :: VnY_zonal_geo(N_heights,N_Latitudes,N_longitudes)
  ! Local
  INTEGER    :: mp, lp
  INTEGER    :: i, ih, ihl, ihu, ii, ilat, ilat1, ilat2, ilon, ilon1, ilon2, itube_loop
  INTEGER    :: ispecial, l, m, n, ilog10, iparam, iconstant_above_top_level
  REAL(prec) :: fac_height, faclat, faclon, glond2
  REAL(prec) :: param_l11, param_l12, param_l21, param_l22,  param_u11, param_u12, param_u21, param_u22
  REAL(prec) :: param_11, param_12, param_21, param_22, param_1, param_2
  REAL(prec) :: gcolat_deg
  REAL(prec) :: glon_deg
  REAL(prec) :: pz_km
    ...
    do mp = grid % mp_low, grid % mp_high
      do lp = 1 , grid % NLP
        DO i = 1 , grid % flux_tube_max(lp)
          ...
          gcolat_deg = rtd * grid % colatitude(i,lp,mp)
          glon_deg   = rtd * grid % longitude(i,lp,mp)
          pz_km      = grid % altitude(i,lp) / 1000.0_prec

          IF ( glon_deg > 360.0_prec ) glon_deg = glon_deg - 360.0_prec
          IF ( glon_deg < 0.0_prec ) glon_deg = glon_deg + 360.0_prec

          ilon2 = 1
          ilon1 = N_longitudes
          ispecial = 1
          DO ilon = 1 , N_longitudes
            IF ( geo_grid_longitudes_degrees(ilon) > glon_deg ) THEN
              ilon2 = ilon
              ilon1 = ilon - 1
              EXIT
            ENDIF
          ...
          DO ilat = 1 , N_Latitudes
            IF ( geo_grid_colatitudes_degrees(ilat) > gcolat_deg ) THEN
              ilat2 = ilat
              ilat1 = ilat - 1
              EXIT
            ENDIF
          ...
          ihl = N_heights - 1
          DO ih = 1 , N_heights
            IF ( fixed_hts_in_km(ih) > PZ_km ) THEN
                ihu = ih
                ihl = ih - 1
                EXIT
            ENDIF
          ...
          fac_height = (PZ_km-fixed_hts_in_km(ihl))/(fixed_hts_in_km(ihu)-fixed_hts_in_km(ihl))

          IF ( ispecial == 0 ) THEN
              faclon = (glon_deg-geo_grid_longitudes_degrees(ilon1)) / &
                       (geo_grid_longitudes_degrees(ilon2)-geo_grid_longitudes_degrees(ilon1))
          ELSEIF ( ispecial == 1 ) THEN
              faclon = (glon_deg-geo_grid_longitudes_degrees(ilon1)) &
              /(geo_grid_longitudes_degrees(ilon2)+360.0_prec-geo_grid_longitudes_degrees(ilon1))
          ENDIF

          faclat = (gcolat_deg-geo_grid_colatitudes_degrees(ilat1)) / &
          ...
          DO iparam = 1, 10
            ...
            IF ((fac_height .ge. 1.0_prec) .and. (iconstant_above_top_level .eq. 1)) THEN
                param_11 = param_u11
                param_12 = param_u12
                param_21 = param_u21
                param_22 = param_u22
            ...
            SELECT CASE (iparam)
              ...
              CASE(1)
                neutrals % oxygen(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(2)
                neutrals % molecular_oxygen(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(3)
                neutrals % molecular_nitrogen(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(4)
                neutrals % hydrogen(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(5)
                neutrals % helium(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(6)
                neutrals % nitrogen(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(7)
                neutrals % temperature(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(8)
                neutrals % temperature_inf(i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(9)
!
! Note: in HWM, wind component 1 is Meridional Northwards and wind component 2 is Zonal Eastwards
!       whereas, in IPE it is the opposite way round.
!       So, HWM(1) -> IPE(2) and HWM(2) -> IPE(1)
!
! neutrals % velocity_geographic(2) is Meridional-Northwards
                neutrals % velocity_geographic(2,i,lp,mp) = ((param_2-param_1)*faclat) + param_1

              CASE(10)
! neutrals % velocity_geographic(1) is Zonal-Eastwards
                neutrals % velocity_geographic(1,i,lp,mp) = ((param_2-param_1)*faclat) + param_1
