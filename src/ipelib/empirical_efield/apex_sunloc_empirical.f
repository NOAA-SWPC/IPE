...
! GHGM
!     IF (DATE .GT. EPOCH(NYT)+5.) WRITE(6,*) 'COFRM empirical ',DATE
...
C          Error trap diagnostics:
 ...
 9200 FORMAT(' ',/,
     ...
     +                          ' recommended for extrapolation.')
      ...
      SUBROUTINE DYPOL_empirical (COLAT,ELON,VP)
...
C          .2 = 2*(10**-4 T/gauss)*(1000 m/km) (2 comes through F0 in COFRM).
      ...
      SUBROUTINE FELDG_empirical (IENTY,GLAT,GLON,ALT, 
     >                            BNRTH,BEAST,BDOWN,BABS)
...
!nm022208:      COMMON /MAGCOF/ NMAX,GB(144),GV(144),ICHG
      ...
      IF (IENTY .EQ. 1) THEN
        ...
        CP   = COS(RLON)
        SP   = SIN(RLON)
        CALL GD2CART_empirical (GLAT,GLON,ALT,XXX,YYY,ZZZ)
        XXX = XXX/RE
        YYY = YYY/RE
        ZZZ = ZZZ/RE
      ELSE
...
C          GV).  Factor .1 converts km to m and gauss to tesla.
      ...
      SUBROUTINE GD2CART_empirical (GDLAT,GLON,ALT,X,Y,Z)
C          Convert geodetic to cartesian coordinates by calling CONVRT
C          940503 A. D. Richmond
      PARAMETER (RTOD=57.2957795130823, DTOR=0.01745329251994330)
      CALL CONVRT_empirical (1,GDLAT,ALT,RHO,Z)
      ANG = GLON*DTOR
      X = RHO*COS(ANG)
      Y = RHO*SIN(ANG)
      RETURN
      ...
      SUBROUTINE CONVRT_empirical (I,GDLAT,ALT,X1,X2)
...
!-----------------------------------------------------------------------      
!-----------------------------------------------------------------------      
      SUBROUTINE SOLGMLON_empirical (XLAT,XLON,COLAT,ELON,MLON)
