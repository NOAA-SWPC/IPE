  ...
  INTEGER :: rc
    ...
    IF( initSuccess )THEN
      ...
      IF( readSuccess )THEN
        ...
        CALL ipe % Write_NetCDF_IPE( "IPE_State.apex."//ipe % time_tracker % DateStamp( )//".nc", rc )

        CALL ipe % Write_to_HDF5( "IPE_State.apex."//ipe % time_tracker % DateStamp( )//".h5", rc )

        CALL ipe % Write_Geographic_NetCDF_IPE( "IPE_State.geo."//ipe % time_tracker % DateStamp( )//".nc", rc )
